...
parameter( SJ = 1.0 , SE = 1.0 , SA = 7.35 )
...
real(kind=8) :: E(Estep+1),ii,Py_ave,Px_ave1,Px_ave2,Px_ave3,Px_ave4
...
! %%%%%%%%%%%%%%%%%! %%%%%%%%%%%%%%%%%! %%%%%%%%%%%%%%%%%! %%%%%%%%%%%%%%%%%
! unpolarized, without field, equilibrium
...
! %%%%%%%%%%%%%%%%%
Px_ave1 = sum( PxD2_1(:,:,N2:N3) ) / (N3-N2+1) / (L*L)
...
! %%%%%%%%%%%%%%%%%! %%%%%%%%%%%%%%%%%! %%%%%%%%%%%%%%%%%! %%%%%%%%%%%%%%%%%
! polarized, with field, equilibrium
! ! Calculate the average polarization
!k=start-1
N2 = 600
N3 = 800
deallocate(Px_his)
deallocate(Py_his)
deallocate(Px_ave_his)
deallocate(Py_ave_his)
deallocate(px_site_his)
deallocate(py_site_his)
deallocate(p_site)
deallocate(p_site_his)
deallocate(Px_2)
deallocate(Py_2)
...
PyD2_1=  PyD2 ( (R+1) : (L+R) , (R+1) : (L+R) , 1:N ) 
! %%%%%%%%%%%%%%%%%
Px_ave2 = sum( PxD2_1(:,:,N2:N3) ) / (N3-N2+1) / (L*L)
...
PyD2_1=  PyD2 ( (R+1) : (L+R) , (R+1) : (L+R) , 1:N ) 
! %%%%%%%%%%%%%%%%%
Px_ave3 = sum( PxD2_1(:,:,N2:N3) ) / (N3-N2+1) / (L*L)
...
PyD2_1=  PyD2 ( (R+1) : (L+R) , (R+1) : (L+R) , 1:N ) 
! %%%%%%%%%%%%%%%%%
Px_ave4 = sum( PxD2_1(:,:,N2:N3) ) / (N3-N2+1) / (L*L)
...
FMT   = "(F10.5, F10.5, F10.5, F10.5, F10.5)"
open( unit = 10, file ='erg.txt',status = 'replace')
do i=1,1
write(10,FMT) Temperature , Px_ave1, Px_ave2 , Px_ave3 , Px_ave4
enddo
close(unit=10)
